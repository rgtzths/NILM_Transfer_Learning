...
class API():
    ...
    def __init__(self,params):
        ...
        self.train_datasets_dict = params['train']
        self.cv_datasets_dict = params.get('cross_validation', None)
        self.test_datasets_dict = params['test']
    ...
    def experiment(self):
            ...
            if self.chunk_size:
                ...
                if clf.chunk_wise_training:
                    ...
                    for i in range(n_epochs):
                        self.train_chunk_wise(clf, self.train_datasets_dict, i)

                else:
                    print ("Joint training for ",clf.MODEL_NAME)
                    self.train_jointly(clf, self.train_datasets_dict, self.cv_datasets_dict)            
        ...
        if self.chunk_size:
            ...
            self.test_chunk_wise(self.test_datasets_dict)

        else:
            print ("Joint Testing for all algorithms")
            self.test_jointly(self.test_datasets_dict)
    ...
    def test_chunk_wise(self,d):
        ...
        for dataset in d["datasets"]:
            print("Loading data for ",dataset, " dataset")
            for building in d["datasets"][dataset]['buildings']:
                test=DataSet(d["datasets"][dataset]['path'])
                test.set_window(start=d["datasets"][dataset]['buildings'][building]['start_time'],end=d["datasets"][dataset]['buildings'][building]['end_time'])
    ...
    def train_jointly(self, clf, train_params, cv_params):
        ...
        for d in train_params["datasets"]:
            print("Loading data for ", d, " dataset")
            dataset=DataSet(train_params["datasets"][d]['path'])

            for building in train_params["datasets"][d]['buildings']:
                print("Loading building ... ",building)
                
                if self.use_activations:
                    print("Getting appliance activations")
                    for appliance_name in self.appliances:
                        dataset.set_window(start=train_params["datasets"][d]['buildings'][building]['start_time'],
                                         end=train_params["datasets"][d]['buildings'][building]['end_time'])

                        activations = dataset.buildings[building].elec[appliance_name].activation_series(
                        ...
                        for period in activations:
                            ...
                            mains_df = next(dataset.buildings[building].elec.mains().load(physical_quantity='power', ac_type=self.power['mains'], sample_period=self.sample_period))
                            
                            if not mains_df.empty:
                                mains_df = mains_df[[list(mains_df.columns)[0]]]

                                appliance_df = next(dataset.buildings[building].elec[appliance_name].load(physical_quantity='power', ac_type=self.power['appliance'], sample_period=self.sample_period))
                    ...
                    if self.DROP_ALL_NANS:
                        mains_df, appliance_readings = self.dropna(mains_df, appliance_readings)

                    if self.artificial_aggregate:
                        print ("Creating an Artificial Aggregate")
                        mains_df = pd.DataFrame(np.zeros(appliance_readings[0].shape),index = appliance_readings[0].index,columns=appliance_readings[0].columns)
                        for app_reading in appliance_readings:
                            mains_df+=app_reading
        ...
        if cv_params is not None:
            for d in cv_params["datasets"]:
                print("Loading data for ", d, " dataset")
                dataset=DataSet(cv_params["datasets"][d]['path'])

                for building in cv_params["datasets"][d]['buildings']:
                    print("Loading building ... ",building)
                    
                    if self.use_activations:
                        print("Getting appliance activations")
                        for appliance_name in self.appliances:
                            dataset.set_window(start=cv_params["datasets"][d]['buildings'][building]['start_time'],
                                            end=cv_params["datasets"][d]['buildings'][building]['end_time'])
                            ...
                            for period in activations:
                                ...
                                if tz_offset_bg > tz_offset_end:
                                    ...
                                    beg = period.index[0].tz_localize(None)
                                    end = period.index[-1].tz_localize(None) - tz_offset_td64
                                else:
                                    beg = period.index[0].tz_localize(None)
                                    end = period.index[-1].tz_localize(None)

                                dataset.set_window(start=beg,end=end)
                                ...
                                if not mains_df.empty:
                                    ...
                                    if appliance_name in self.cv_data:
                                        self.cv_data[appliance_name]["mains"].append(mains_df)
                                        self.cv_data[appliance_name]["appliance"].append(appliance_df[0])
                                    else:
                                        self.cv_data[appliance_name] = { "mains" : [mains_df], "appliance" : appliance_df}
                        ...
                        for i, appliance_name in enumerate(self.appliances):
                            if appliance_name in self.cv_data:
                                self.cv_data[appliance_name]["mains"].append(mains_df)
                                self.cv_data[appliance_name]["appliance"].append(appliance_readings[i])
                            else:
                                self.cv_data[appliance_name] = { "mains" : [mains_df], "appliance" : [appliance_readings[i]]}
                    
            clf.partial_fit(self.train_data, self.cv_data)
        else:
            clf.partial_fit(self.train_data, None)

    
    def test_jointly(self,test_params):
        
        # store the test_main readings for all buildings
        for d in test_params["datasets"]:

            print("Loading data for ", d, " dataset")
            dataset=DataSet(test_params["datasets"][d]['path'])
            
            for building in test_params["datasets"][d]['buildings']:
                self.test_data = {}
                
                if self.use_activations:
                    print("Getting appliance activations")
                    for appliance_name in self.appliances:
                        dataset.set_window(start=test_params["datasets"][d]['buildings'][building]['start_time'],
                                         end=test_params["datasets"][d]['buildings'][building]['end_time'])
                        ...
                        for period in activations:
                            ...
                            test_mains=next(dataset.buildings[building].elec.mains().load(physical_quantity='power', ac_type=self.power['mains'], sample_period=self.sample_period))
                    ...
                    self.call_predict(self.classifiers, dataset.metadata["timezone"])

                else:
                    dataset.set_window(start=test_params["datasets"][d]['buildings'][building]['start_time'],
                                       end=test_params["datasets"][d]['buildings'][building]['end_time'])
                    test_mains=next(dataset.buildings[building].elec.mains().load(physical_quantity='power', ac_type=self.power['appliance'], sample_period=self.sample_period))
                    ...
                    if self.site_only != True:
                        ...
                        if self.artificial_aggregate:
                            ...
                            for app_readings in appliance_readings:
                                test_mains+=app_readings
